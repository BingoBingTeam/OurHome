package com.pager.indicator.circlepagerindctor;
...
public class CircleIndctor extends View {
	private final String TAG = CircleIndctor.class.getSimpleName();

	private ViewPager viewPager;
	private List<ShpaeHolder> tabItems;
	private ShpaeHolder movingItem;
	...
	public CircleIndctor(Context context) {
		super(context);
		init(context, null);
	}

	public CircleIndctor(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	public CircleIndctor(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void handleTypedArray(Context context, AttributeSet attrs) {
		...
		TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CircleIndctor);
		mIndicatorRadius = typedArray.getDimensionPixelSize(R.styleable.CircleIndctor_ci_radius, DEFAULT_INDICATOR_RADIUS);
		mIndicatorMargin = typedArray.getDimensionPixelSize(R.styleable.CircleIndctor_ci_margin, DEFAULT_INDICATOR_MARGIN);
		mIndicatorBackground = typedArray.getColor(R.styleable.CircleIndctor_ci_background, DEFAULT_INDICATOR_BACKGROUND);
		mIndicatorSelectedBackground = typedArray.getColor(R.styleable.CircleIndctor_ci_selected_background, DEFAULT_INDICATOR_SELECTED_BACKGROUND);
		int gravity = typedArray.getInt(R.styleable.CircleIndctor_ci_gravity, DEFAULT_INDICATOR_LAYOUT_GRAVITY);
		mIndicatorLayoutGravity = Gravity.values()[gravity];
		int mode = typedArray.getInt(R.styleable.CircleIndctor_ci_mode, DEFAULT_INDICATOR_MODE);
	...
	private void trigger(int position, float positionOffset) {
		CircleIndctor.this.mCurItemPosition = position;
		CircleIndctor.this.mCurItemPositionOffset = positionOffset;
	...
	private void createTabItems() {
		...
		for (int i = 0; i < viewPager.getAdapter().getCount(); i++) {
			...
			ShpaeHolder shapeHolder = new ShpaeHolder(drawable);
	...
	private void createMovingItem() {
		OvalShape circle = new OvalShape();
		ShapeDrawable drawable = new ShapeDrawable(circle);
		movingItem = new ShpaeHolder(drawable);
		Paint paint = drawable.getPaint();
	...
	private void layoutTabItems(final int containerWidth, final int containerHeight) {
		...
		for (int i = 0; i < tabItems.size(); i++) {
			ShpaeHolder item = tabItems.get(i);
	...
	private void layoutMovingItem(final int position, final float positionOffset) {
		...
		ShpaeHolder item = tabItems.get(position);
	...
	private void drawItem(Canvas canvas, ShpaeHolder shapeHolder) {
		canvas.save();
